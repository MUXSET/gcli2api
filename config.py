...
async def get_oauth_client_id() -> str:
    ...
    val = await get_config_value("google_oauth_client_id", env_var="GOOGLE_OAUTH_CLIENT_ID")
    return str(val) if val else DEFAULT_CLIENT_ID

async def get_oauth_client_secret() -> str:
    """Get OAuth Client Secret from env or config, fallback to default."""
    val = await get_config_value("google_oauth_client_secret", env_var="GOOGLE_OAUTH_CLIENT_SECRET")
    return str(val) if val else DEFAULT_CLIENT_SECRET
...
# Legacy password functions removed (get_api_password, get_panel_password, get_server_password)
# System now uses multi-user authentication with API Keys (sk-gcli-xxx)
...
async def get_credential_isolation_mode() -> bool:
    """
    Get credential isolation mode setting.

    凭证隔离模式：
    - True: 用户只能使用自己上传的凭证，必须使用个人API Key调用接口。
    - False: 所有用户共享凭证池，可以使用全局密码调用接口。

    Environment variable: CREDENTIAL_ISOLATION_MODE
    TOML config key: credential_isolation_mode
    Default: False
    """
    env_value = os.getenv("CREDENTIAL_ISOLATION_MODE")
    if env_value:
        return env_value.lower() in ("true", "1", "yes", "on")

    return bool(await get_config_value("credential_isolation_mode", False))
