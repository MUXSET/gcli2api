...
class CredentialManager:
    ...
    async def _load_credential_by_name(self, filename: str) -> Optional[Tuple[str, Dict[str, Any]]]:
        """加载指定名称的凭证数据"""
        try:
            # 从存储适配器加载凭证数据
            credential_data = await self._storage_adapter.get_credential(filename)
            if not credential_data:
                log.error(f"Failed to load credential data for: {filename}")
                return None

            # 检查refresh_token
            if "refresh_token" not in credential_data or not credential_data["refresh_token"]:
                log.warning(f"No refresh token in {filename}")
                return None
            ...
            if "type" not in credential_data and all(
                ...
                log.debug(f"Auto-added 'type' field to credential from file {filename}")
            ...
            # 缓存当前凭证信息 (仅当是当前使用的凭证时才更新，这里简化处理，总是更新最后一次加载的)
            self._current_credential_file = filename
            self._current_credential_data = credential_data

            return filename, credential_data

        except Exception as e:
            log.error(f"Error loading credential {filename}: {e}")
            return None
    ...
    async def _load_current_credential(self) -> Optional[Tuple[str, Dict[str, Any]]]:
        ...
        if not self._credential_files:
            return None
        return await self._load_credential_by_name(self._credential_files[0])
    ...
    async def get_valid_credential(self, user_id: Optional[str] = None) -> Optional[Tuple[str, Dict[str, Any]]]:
        """
        获取有效的凭证，自动处理轮换、失效凭证切换和冷却检查
        
        Args:
            user_id: 可选的用户ID。如果提供，只从该用户的凭证中选择。
        """
        async with self._operation_lock:
            if not self._credential_files:
                await self._discover_credentials()
            ...
            # 确定要使用的凭证列表
            candidate_files = self._credential_files
            is_isolated = False
            
            if user_id:
                prefix = f"u_{user_id}_"
                candidate_files = [f for f in self._credential_files if f.startswith(prefix)]
                is_isolated = True
                
                if not candidate_files:
                    log.warning(f"User {user_id} has no credentials uploaded.")
                    return None

            # 如果是隔离模式，我们不修改全局轮换队列，只是遍历查找可用凭证
            if is_isolated:
                for current_file in candidate_files:
                    try:
                        # 检查冷却
                        if await self._is_credential_in_cooldown(current_file):
                            log.debug(f"User credential {current_file} is in cooldown, skipping.")
                            continue
                            
                        # 加载凭证
                        result = await self._load_credential_by_name(current_file)
                        if result:
                            return result
                            
                        # 加载失败，标记禁用（这里是否要禁用值得商榷，暂时保持一致）
                        log.warning(f"User credential invalid: {current_file}")
                        # await self.set_cred_disabled(current_file, True) # 隔离模式下暂不自动禁用，以免误伤
                        
                    except Exception as e:
                        log.error(f"Error checking user credential {current_file}: {e}")
                        continue
                
                log.warning(f"No valid credentials found for user {user_id}")
                return None
    ...
    async def record_api_call_result(
        ...
        Args:
            ...
            user_id: 调用者ID（用于错误日志）
            error_msg: 错误详情（用于错误日志）
        """
        try:
            state_updates = {}
            ...
            if success:
                ...
                # 记录详细错误日志
                if error_msg:
                     error_logger.log_error(credential_name, user_id, error_code, error_msg)
