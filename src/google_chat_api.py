...
from src.usage_stats import record_successful_call
...
async def _get_next_credential(
    ...
    """获取下一个可用凭证并准备请求参数"""
    new_credential_result = await credential_manager.get_valid_credential(user_id=user_id)
    ...
    if should_auto_ban:
        ...
        if retry_enabled and attempt < max_retries:
            ...
            result = await _get_next_credential(
                credential_manager, payload, use_public_api, target_url, user_id
            )
            if result:
...
async def send_gemini_request(
    payload: dict, is_streaming: bool = False, credential_manager: CredentialManager = None, user_id: str = None
) -> Response:
    """
    ...
    # 获取当前凭证
    try:
        credential_result = await credential_manager.get_valid_credential(user_id=user_id)
            ...
            if is_streaming:
                ...
                try:
                    ...
                    if resp.status_code == 429:
                        ...
                        if retry_429_enabled and attempt < max_retries:
                            ...
                            if credential_manager:
                                ...
                                new_credential_result = (
                                    await credential_manager.get_valid_credential(user_id=user_id)
                                )
                                if new_credential_result:
                                    current_file, credential_data = new_credential_result
                                    headers, updated_payload, target_url = (
                        ...
                        # 使用统一的错误处理和重试逻辑
                        should_retry, retry_data = await _handle_error_with_retry(
                            credential_manager,
                            resp.status_code,
                            current_file,
                            payload,
                        ...
                        if should_retry and retry_data:
                            # 解包新的请求参数并继续重试
                            current_file, credential_data, headers, final_post_data, target_url = retry_data
                            continue
                ...
                async with http_client.get_client(timeout=None) as client:
                    ...
                    if resp.status_code == 200:
                        return await _handle_non_streaming_response(
                            resp, credential_manager, payload.get("model", ""), current_file, user_id, request_start_time
                        )
...
def _handle_streaming_response_managed(
        ...
        async def cleanup_and_error():
            ...
            if credential_manager and current_file:
                await credential_manager.record_api_call_result(
                    current_file, False, resp.status_code, cooldown_until,
                    user_id=user_id,
                    error_msg=f"{resp.status_code}: {response_content[:500]}" if response_content else f"Error {resp.status_code}"
                )
                
            # Log error call
            try:
                # Calculate latency if start time available
                lat = (time.time() - request_start_time) * 1000 if request_start_time else 0
                call_logger.log_call(current_file, user_id, model_name, lat, resp.status_code, error=response_content[:200] if response_content else "Stream Init Error")
            except Exception:
                pass
    ...
    # Success case (200 OK)
    # Log success call (Stream Start)
    try:
        lat = (time.time() - request_start_time) * 1000 if request_start_time else 0
        call_logger.log_call(current_file, user_id, model_name, lat, 200)
    except Exception:
        pass
                ...
                if not success_recorded:
                    if current_file and credential_manager:
                        ...
                        try:
                            ...
                            await record_successful_call(current_file, latency_ms, model_name, user_id)
                        except Exception as e:
...
async def _handle_non_streaming_response(
    ...
    if resp.status_code == 200:
        try:
            ...
            if current_file and credential_manager:
                ...
                try:
                    latency_ms = (time.time() - request_start_time) * 1000 if request_start_time else 0
                    await record_successful_call(current_file, latency_ms, model_name, user_id)
                    call_logger.log_call(current_file, user_id, model_name, latency_ms, 200)
                except Exception as e:
                    log.debug(f"Failed to record usage statistics: {e}")
        ...
        try:
            lat = (time.time() - request_start_time) * 1000 if request_start_time else 0
            call_logger.log_call(current_file, user_id, model_name, lat, resp.status_code, error=response_content[:200] if response_content else "Error")
        except Exception:
            pass
